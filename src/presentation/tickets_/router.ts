...
export class TicketRoutes_ {
  static get routes(): Router {
    ...
    ); // traer información del ticket
    router.get(
      "/user/:id",
      [AuthMiddlewre.validateJWT],
      ticketController.getTicketsByUser
    ); // traer información de los tickets por id de usuario
    router.post(
      "/",
      [AuthMiddlewre.validateJWT],
    ...
    router.get(
      "/position/:id",
      [AuthMiddlewre.validateJWT],
      ticketController.calculatePositionTime
    ); // id de ticket para saber la posición y el tiempo de atención
    router.get(
      "/inprogress/:id",
      [AuthMiddlewre.validateJWT],
      ticketController.getTicketInProgressByHeadquarter
    ); // id de la sede para saber los tickets en progreso
